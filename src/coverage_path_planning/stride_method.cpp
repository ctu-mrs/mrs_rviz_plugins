...
void StrideMethod::setPolygon(mrs_lib::Polygon &new_polygon, bool update){
...
void StrideMethod::setAngle(int angle, bool update) {
  angle_ = angle;
  if(update){
    drawGrid();
  }
...
void StrideMethod::setOverlap(float percentage, bool update) {
  overlap_ = percentage;
  if(update){
    drawGrid();
  }
...
void StrideMethod::setHeight(float height, bool update) {
  height_ = height;
  if(update){
    drawGrid();
  }
...
void StrideMethod::twistChanged(){
  drawGrid();
}
