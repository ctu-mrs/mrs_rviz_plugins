...
void WaypointPlanner::onPoseSet(double x, double y, double theta) {
  ...
  if (!tf) {
    ...
    if (!point_transformed) {
      setStatus("[Waypoint planner]: Unable to transform cmd reference from body frame to current frame. Z component of visual entity is set to null");
      ROS_INFO("[Waypoint planner]: Unable to transform cmd reference from %s to %s at time %.6f. Z component of visual entity is set to null", source_frame.c_str(), context_->getFrameManager()->getFixedFrame().c_str(),
                ros::Time::now().toSec());
      return;
    }else {
